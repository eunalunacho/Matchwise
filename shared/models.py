...
class Job(BaseModel):
    ...
    last_reported_at: Optional[datetime] = Field(None, description="마지막 리포트 발송 일시")
...
class ApplicationRecord(BaseModel):
    ...
    # 일정 관리
    deadline: Optional[datetime] = None  # 현재 단계의 마감일
    last_notified_at: Optional[datetime] = None # 에이전트가 마지막으로 말 건 시간
    
    # 히스토리 및 노트
    history: List[StatusLog] = Field(default_factory=list)
    notes: str = "" # 내가 쓴 메모
    agent_tips: List[str] = [] # 에이전트가 조사해준 기술 면접 팁 등
...
class CompanyDetail(BaseModel):
    ...
    location: Optional[str] = Field(None, description="본사 소재지")

    # 2. 규모 및 재무 (Scale)
    revenue: Optional[str] = Field(None, description="최근 매출액 (예: 500억)")
    employee_count: Optional[int] = Field(None, description="전체 직원 수")
    funding_stage: Optional[str] = Field(None, description="투자 단계 (예: Series B, 상장)")
